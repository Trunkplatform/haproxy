...
class Haproxy(object):
    ...
    def _config_ssl(self):
        ...
        if certs:
            ...
            self.ssl = "accept-proxy ssl crt /certs/" \
                if self.envvar_proxy_protocol and self.envvar_proxy_protocol.upper() == "TRUE" \
                else "ssl crt /certs/"
    ...
    def _config_global_defaults(cls):
        ...
        cfg["frontend monitor"] = ["bind :%s" % cls.envvar_monitor_port,
                                   "mode http",
                                   "monitor-uri %s" % cls.envvar_monitor_uri]
